...
class Accounts extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $data["base_url"] = base_url(ADMINURI.'accounts/delete_all');
    ...
    public function types()
    {
        ...
        if (!empty($post)) {
            if ($this->uri->segment(4) == "add") {
                ...
                redirect(ADMINURI . '/accounts/types');
            } else {
            ...
            if (empty($this->uri->segment(4))) {
                ...
                $xcrud->unset_view();
                $xcrud->unset_add();
                $xcrud->button(base_url() . ADMINURI . 'accounts/types/{id}', 'Edit', '', 'btn btn-warning', array('target' => '_blank'));
                ...
                $data['content'] = $xcrud->render();
                $data['add_link'] = base_url() . ADMINURI . 'accounts/types/add';
                $data['main_content'] = 'Admin/xcrud';
                $data['crumbdata'] = array('Accounts','Types');
                $data['crumb'] = 'Admin/crumb';
                $data['base_url'] = base_url(ADMINURI.'accounts/delete_all_account_type');
