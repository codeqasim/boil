...
class Xcrud
{
    ...
    { // loads themes configuration from ini file
        if (is_file(XCRUD_PATH . '/' . Xcrud_config::$themes_path . '/' . $this->theme . '/xcrud.ini'))
            ...
            self::error('xcrud.ini does not exist in your themes folder');
    }
    ...
    public static function load_js()
    {
        ...
        if (Xcrud_config::$load_googlemap)
            $out .= '<script src="//maps.google.com/maps/api/js?key=AIzaSyBb53sK1dTXpEZYkPgKYyn0uxiHmI_voJk&language=' . $language . '"></script>';
    ...
    protected function add_button($class = '', $icon = '')
    {
        ...
        {
            $ab =  "<button class='btn btn-danger' id='deleteAll'>Delete All</button> ".$this->render_button('add', 'create', '', $class, $icon) ;
            return $ab;
        }
    }
